class CheckingAccountController < ApplicationController
  ...
  def delete
    ...
    if account == nil
      redirect_to "/user/#{user.id}", alert: "You need to select an account to delete!"
    else
      if user.main_account_id == account.bankaccount_number
      redirect_to "/user/#{user.id}", alert: "You cannot delete your main account"
      else
      main_account.balance = main_account.balance + account.balance
      main_account.save
      account.delete
      redirect_to "/user/#{user.id}", notice: "Account delete. The remaining balance has been transfered to your main account"
      end
    end
  end
  ...
  def create
    ...
    if u != nil
      if u.authenticate(params["password"])
        ...
        redirect_to "/user/#{user.id}", notice: "Account Successfully Created!
